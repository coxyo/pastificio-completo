...
const BackupManager = () => {
  ...
  const checkDriveStatus = async () => {
    try {
      ...
      setNextAutoBackup('Oggi alle 02:00'); // Il cron job è schedulato alle 2 AM
    } catch (error) {
  ...
  const loadDriveBackups = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setBackupsDrive(response.data.data || []);
        
        // Aggiorna ultimo backup da Drive
  ...
  const downloadDriveBackup = async (backup) => {
    try {
      ...
      // In produzione, dovresti implementare un endpoint di download
      const response = await axios.get(
        `${API_URL}/backup/download/${backup.id}`,
        { responseType: 'blob' }
      );
  ...
  const deleteDriveBackup = async (backupId) => {
    ...
    try {
      await axios.delete(`${API_URL}/backup/${backupId}`);
  ...
  const performLocalBackup = async () => {
    ...
    try {
      ...
      const newBackups = [backupInfo, ...backupsLocal].slice(0, 10); // Mantieni solo ultimi 10
