// src/components/TabellaOrdiniCompleta.js
...
const TabellaOrdiniCompleta = ({ ordini, onModifica, onElimina, onDettagli, onCondividi }) => {
  ...
  const getBadgeStato = (stato) => {
    const stati = {
      ...
      consegnato: { bg: 'bg-gray-100', text: 'text-gray-800', label: 'Consegnato' },
            ...
            ordini.map((ordine) => (
              <tr key={ordine._id || ordine.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="font-medium">{ordine.cliente?.nome || 'N/A'}</div>
                    ...
                    <div>
                      {ordine.prodotti.slice(0, 2).map((p, idx) => (
                        <div key={idx} className="text-xs">
                          • {p.nome || p.nomeProdotto} ({p.quantita} {p.unita})
                        </div>
                      ))}
