// src/components/GestioneLimiti.js - ✅ AGGIORNAMENTO 10/03/2026: Tab Limiti Periodo
'use client';
...
import GestioneLimitiPeriodo from './GestioneLimitiPeriodo';
...
export default function GestioneLimiti() {
  ...
  // ── Limiti giornalieri state (invariato) ──
  ...
  // ── Funzioni limiti giornalieri (invariate) ──
  ...
  const creaLimitiPeriodo = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/limiti/bulk`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ limiti, sovrascrivi: formPeriodo.sovrascrivi })
      });
      if (response.ok) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {/* Bottoni contestuali alla tab attiva */}
        {tabAttiva === 0 && (
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        <Tabs
          value={tabAttiva}
          onChange={(_, val) => setTabAttiva(val)}
          textColor="primary"
        ...
        >
          <Tab
            ...
            iconPosition="start"
            label="Limiti Giornalieri"
            sx={{ fontWeight: 600 }}
          />
          <Tab
            icon={<DateRangeIcon sx={{ fontSize: 18 }} />}
            iconPosition="start"
            label="Limiti Periodo"
            sx={{ fontWeight: 600 }}
          />
        </Tabs>
      </Box>
      ...
      {/* ══════════════════════════════════════════════════
          TAB 0 — LIMITI GIORNALIERI (contenuto invariato)
          ══════════════════════════════════════════════════ */}
      {tabAttiva === 0 && (
        <>
          {/* Statistiche */}
      ...
      {/* ══════════════════════════════════════════════════
          TAB 1 — LIMITI PERIODO (GestioneLimitiPeriodo)
          ══════════════════════════════════════════════════ */}
      {tabAttiva === 1 && (
        <GestioneLimitiPeriodo />
      )}

      {/* ── Dialog Nuovo Limite Singolo (invariato) ── */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}>
              <Typography variant="body2" color="text.secondary" align="center">OPPURE</Typography>
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth>
      ...
      {/* ── Dialog Limiti per Periodo (invariato) ── */}
      <Dialog open={dialogPeriodoOpen} onClose={() => setDialogPeriodoOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              ...
              >
                {Object.entries(PRESET_PERIODI).map(([key, preset]) => (
                  <FormControlLabel key={key} value={key} control={<Radio />} label={preset.nome} />
                ))}
              </RadioGroup>
            ...
            <Grid item xs={6}>
              <TextField
                label="Data Inizio" type="date" fullWidth
            ...
            <Grid item xs={6}>
              <TextField
                label="Data Fine" type="date" fullWidth
            ...
            <Grid item xs={8}>
              <TextField
                label="Limite Quantità (per giorno)" type="number" fullWidth
            ...
            <Grid item xs={12}>
              <TextField
                label="Soglia Allerta (%)" type="number" fullWidth
            ...
            <Grid item xs={12}>
              <TextField
                label="Note" fullWidth multiline rows={2}
            ...
            <Grid item xs={12}>
              ...
              <Typography variant="caption" color="text.secondary" display="block">
                Se già esistono limiti per alcune date, verranno aggiornati invece di creare duplicati
              </Typography>
            </Grid>
          </Grid>
        </DialogContent>
